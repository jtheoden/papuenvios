...
export const getConversionRate = async (fromCurrencyId, toCurrencyId) => {
  try {
    ...
    const fetchOfficialRate = async (currencyCode) => {
      try {
        ...
        if (error) {
          ...
          if (['42P01', 'PGRST116', 'PGRST204', '404'].includes(parsed?.code) || parsed?.status === 404) {
            return OFFICIAL_RATE_FALLBACKS[currencyCode] ?? null;
          }
          // Any other error also falls back to static values while logging
          logError(parsed, { operation: 'getConversionRate - officialRateFallback', currency: currencyCode });
          return OFFICIAL_RATE_FALLBACKS[currencyCode] ?? null;
        }
    ...
    const fromRateRaw = await fetchOfficialRate(fromCurr.code);
    const toRateRaw = await fetchOfficialRate(toCurr.code);

    const fromRate = fromRateRaw ?? OFFICIAL_RATE_FALLBACKS[fromCurr.code] ?? 1;
    const toRate = toRateRaw ?? OFFICIAL_RATE_FALLBACKS[toCurr.code] ?? 1;

-- ============================================================================
-- EMERGENCY FIX FOR DATABASE TIMEOUTS - NO MIGRATION TRACKING NEEDED
-- ============================================================================
...
-- We'll re-enable with correct policies below
...
-- COMBOS table (if it exists separately)
DO $$
BEGIN
    IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'combos') THEN
        EXECUTE 'CREATE POLICY "combos_select_public" ON combos FOR SELECT USING (is_active = true OR is_admin_user())';
        EXECUTE 'CREATE POLICY "combos_all_admin" ON combos FOR ALL USING (is_admin_user())';
    END IF;
END $$;
